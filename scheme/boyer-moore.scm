...
;;; This code is specific to MIT Scheme, but only in the use of octet
;;; vectors.  To port it to other Schemes, simply substitute the
;;; appropriate procedures for octet vectors.
...
(define (vector-8b-forward-search-cache/bm pattern start end)
  (let ((max-suffix (compute-bm-max-suffix pattern start end)))
    (make-bm-cache
     ...
     (compute-bm-good-suffix-table
      (- end start)
      max-suffix
      (compute-bm-prefix-table (vector-8b-reverse pattern start end)
                               0
                               (- end start)))
     max-suffix)))

(define (vector-8b-backward-search-cache/bm pattern start end)
  (let ((nrettap (vector-8b-reverse pattern start end))
...
(define (vector-8b-reverse v8b start end)
  (let* ((length (- end start))
         (v8b* (make-vector-8b length)))
    (let loop ((index start) (index* length))
      (if (< index end)
          (let ((index* (- index* 1)))
            (vector-8b-set! v8b* index* (vector-8b-ref v8b index))
            (loop (+ index 1) index*))))
    v8b*))

(define (vector-8b-search-forward/bm instance instance-start instance-end
                                     cache)
...
(define (vector-8b-search-backward/bm instance instance-start instance-end
                                      cache)
      ...
      (if (> scan-index scan-limit)
          ...
          (let match ((match-index (+ scan-index pattern-length))
            ...
            (let ((match-index (- match-index 1))
              ...
              (let ((match-octet (vector-8b-ref instance match-index))
                    (pattern-octet (vector-8b-ref pattern pattern-index)))
    ...
    (let scan-pattern ((offset 1) (prefix-length 0))
      (if (< offset pattern-length)
          ((lambda (prefix-length)
           ...
           (let ((octet (vector-8b-ref pattern (+ start offset))))
             (let find-prefix ((prefix-length prefix-length))
               (cond ((= octet
                         (vector-8b-ref pattern
                                        (+ start prefix-length)))
                      (+ prefix-length 1))
                     ((= prefix-length 0) 0)
                     (else
...
(define-integrable (bm-skip-forward pattern-start pattern-index match-octet
                                    last-occurrence-table
                                    good-suffix-table)
...
(define-integrable (bm-skip-backward pattern-end pattern-index match-octet
                                     last-occurrence-table
                                     good-suffix-table)
