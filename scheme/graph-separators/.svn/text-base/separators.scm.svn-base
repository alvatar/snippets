...
(module graph-separators

 (graph-separation-vertices)
...
(define (graph-separation-vertices g . rest)
  (let-optionals rest ((slot (lambda (n) (car ((g 'node-info) n))))
		       (set-slot! (lambda (n v) (set-car! ((g 'node-info) n) v))))
  (let ((nodes (map car ((g 'nodes))))
	(succ  (g 'succ))
	(pred  (g 'pred)))
    (let ((to  (lambda (n) (append (succ n) (pred n)))))
      (separation-vertices nodes to slot set-slot!)))))
...
; A graph is said to have a separation vertex v (sometimes called an
; articulation point) if there exist vertices a and b, where a \neq b
; and b \neq v, and all paths connecting a and b pass through v.  A
; graph which has a separation vertex is called separable, and one
; which has none is called non-separable.
...
(define (find-unused-edge v i stack start v-res c-res)
  (let ((e (find edge-unused? (vertex-edges v))))
    (if e
...
(define (gather-nonseparable-with-start v i stack start v-res c-res)
  (receive (vertices stack)
    ...
    (let* ((data (vertex-data start))
      ...
      (if (not (find edge-unused? (vertex-edges start)))
	  (values v-res c-res)
