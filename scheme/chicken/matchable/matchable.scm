...
;; 2008/03/22 - superfluous right-paren in "match-syntax-error" def
...
;; low-level macro system is available.
...
(cond-expand
 ...
 (srfi-46
  (define-syntax match-check-ellipse
	...
	((_ x sk fk) fk))) )
 (chicken
  (define-syntax (match-check-ellipse x r c)
    (let ((sk (caddr x))
	  (fk (cadddr x)))
      (if (c (r '...) (cadr x))
	  sk
        ...
        (lambda (closing-environment)
          (make-syntactic-closure usage-environment '()
            (if (identifier? (cadr form))
                ...
                (cadddr form)))))))))
 (chicken
  (define-syntax (match-check-identifier x r c)
    (let ((sk (caddr x))
	  (fk (cadddr x)))
      (if (symbol? (cadr x))
	  sk
...
;; And another portability headache that I'm punting for now.

;; Convert `..k' patterns (not used much, and under-powered, worry
;; about this later).

;; (cond-expand
;;  (syntax-case
;;   (define-syntax (match-convert-..k stx)
;;     (syntax-case stx ()
;;       ((_ (p q r s) (sk ...) fk)
;;        (let ((q2 (cadadr (syntax-object->datum stx))))
;;          (if (symbol? q2)
;;            (let* ((str (symbol->string q2))
;;                   (len (string-length str)))
;;              (cond
;;                ((and (> len 2)
;;                      (eqv? #\. (string-ref str 0))
;;                      (eqv? #\. (string-ref str 1))
;;                      (string->number (substring str 2 len)))
;;                 => (lambda (n)
;;                      (syntax (sk ... n))))
;;                (else
;;                 (syntax fk))))
