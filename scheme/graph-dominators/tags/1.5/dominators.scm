...
; Adapted for Chicken Scheme by Ivan Raikov.

(module graph-dominators

 (graph-find-dominators-quickly! graph-find-dominators-slowly!)
...
; In a CFG, block M dominates block N if every path from the entry
; that reaches block N has to pass through block M. The entry block
; dominates all blocks.
;
; Block M immediately dominates block N if M dominates N, and there is
; no intervening block P such that M dominates P and P dominates N. In
; other words, M is the last dominator on any path from entry to
; N. Each block has a unique immediate dominator, if it has any at
; all.
...
(define (graph-find-dominators-quickly! g . rest)
  (let-optionals rest ((slot (lambda (n) (car ((g 'node-info) n))))
		       (set-slot! (lambda (n v) (set-car! ((g 'node-info) n) v))))
  (let ((roots ((g 'roots)))
	(succ  (g 'succ))
	(pred  (g 'pred)))
    (match roots
	   ((root) (find-dominators-quickly! root succ pred slot set-slot!))
	   (else   (error 'graph-find-dominators-quickly! "given graph must have exactly one root"))))))
...
(define (graph-find-dominators-slowly! g . rest)
  (let-optionals rest ((slot (lambda (n) (car ((g 'node-info) n))))
		       (set-slot! (lambda (n v) (set-car! ((g 'node-info-set!) n) v))))
  (let ((roots ((g 'roots)))
	(succ  (g 'succ))
	(pred  (g 'pred)))
    (match roots
	   ((root) (find-dominators-slowly! root succ pred slot set-slot!))
	   (else   (error 'graph-find-dominators-slowly! "given graph must have exactly one root"))))))
