...
;;
;; $Id: wheelbarrow.scm,v 1.1 2003/07/05 17:00:39 foof Exp $
...
(define (main args)
  (gtk-init args)
  (let-objects
   (window window :title "Table" :parent GTK_WINDOW_POPUP
           :delete (lambda _ (gtk-main-quit) #f))
   (let* ((style (gtk-widget-get-default-style))
     ...
     (receive (gdk-pixmap mask)
       ...
       (let-objects
        (fixed fixed :width-request 200 :height-request 200
         (pixmap :value (gtk-image-new-from-pixmap gdk-pixmap mask)
                 :fixed-x 0 :fixed-y 0))
