...
(module graph-ssa
 ...
 (graph->ssa-graph graph-ssa-find-joins)

 (import scheme chicken data-structures)

 (require-extension srfi-1 matchable graph-dominators )


(define (graph->ssa-graph g)
  (let ((roots  ((g 'roots)))
	(tempv  (make-vector ((g 'order)) #f)))
    (match roots
	   ((root) (graph->ssa-graph! root (g 'succ) (lambda (i) (vector-ref tempv i))
				      (lambda (i x) (vector-set! tempv i x))))
	   (else   (error 'graph->ssa-graph "given graph must have exactly one root")))
      (vector->list tempv )))
...
(define (node-dispatch nodes)
  (lambda (x)
    (case x
      ((roots) (lambda () (list (car nodes))))
...
(define (graph->ssa-graph! root successors temp set-temp!)
  (let ((graph (real-graph->ssa-graph root successors temp set-temp!)))
    (graph-find-dominators-quickly! (node-dispatch graph)
...
(define (graph-ssa-find-joins nodes)
  (map node-data (really-find-joins nodes)))
