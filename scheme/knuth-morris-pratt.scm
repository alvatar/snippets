...
(define (vector-8b-forward-search-cache/kmp pattern start end)
  (make-kmp-cache pattern start
                  (compute-kmp-prefix-table pattern start end)))

(define (vector-8b-backward-search-cache/kmp pattern start end)
  (make-kmp-cache pattern start
                  (compute-kmp-suffix-table pattern start end)))

(define (vector-8b-search-forward/kmp instance instance-start instance-end
                                      cache)
...
(define (vector-8b-search-backward/kmp instance instance-start instance-end
                                       cache)
            ...
            (else
             (loop (+ instance-index 1)
                   (kmp-step-forward pattern
                                     pattern-start
                                     pattern-index
                                     (vector-8b-ref instance instance-index)
                                     prefix-table)))))))
          ...
          (else
           (loop (- instance-index 1)
                 (kmp-step-backward pattern
                                    pattern-start
                                    pattern-index
                                    (vector-8b-ref instance
                                                   (- instance-index 1))
                                    suffix-table))))))

(define-integrable (kmp-step-forward pattern pattern-start pattern-index octet
                                     prefix-table)
  (let loop ((pattern-index pattern-index))
    (cond ((= octet (vector-8b-ref pattern pattern-index))
           (+ pattern-index 1))
...
(define-integrable (kmp-step-backward pattern pattern-start pattern-index octet
                                      suffix-table)
  (let loop ((pattern-index pattern-index))
      ...
      (if (< index (- end 1))
          (let find-prefix ((prefix-index prefix-index))
            (cond ((not prefix-index)
                   (let ((index* (+ index 1)))
                     (if (not (= (vector-8b-ref pattern index*)
                                 (vector-8b-ref pattern start)))
                         (store-prefix! index* start))
                     (scan-pattern index* start)))
                  ((= (vector-8b-ref pattern index)
                      (vector-8b-ref pattern prefix-index))
                   (let ((index* (+ index 1))
    ...
    (let scan-pattern ((index end) (suffix-index #f))
      (if (> index (+ start 1))
          (let find-suffix ((suffix-index suffix-index))
            (if (not suffix-index)
                (let ((index (- index 1)))
                  (if (not (= (vector-8b-ref pattern (- index 1))
                              (vector-8b-ref pattern (- end 1))))
                      (store-suffix! index end))
                  (scan-pattern index end))
                (if (= (vector-8b-ref pattern (- index 1))
                       (vector-8b-ref pattern (- suffix-index 1)))
                    (let ((index* (- index 1))
