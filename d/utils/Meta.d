...
private char[] genPrivAndProp(char[] a){
	char[] buf1;
	int i;
	int count=0;
...
public char[] createPrivAndProp(char[] a){
	char[] r;
	int last=0;
	foreach(ln, c; a){
		if(c==','){
...
char[] resolveAutoOverrideCodegen(char[] thisType, T ...)() {
	char[] res = "";
	foreach (Field; T) {
...
struct ApplyEachProxy(Arg, char[] sc=``) {
	Arg[] args;
	...
	ApplyEachProxy!(Arg, sc~`.`~mname) member(char[] mname)() {
...
/**
	---
	applyEach(input.params).member!(`semantic`) >> (char[] name, int idx) {
	}
	---
*/
...
private char[] indexedItemsString(char[] thingyName, int num) {
	char[] res;
	for (int i = 0; i < num; ++i) {
		if (i > 0) res ~= `,`;
...
struct EachMemberProxy(char[] mname, Iterable) {
	Iterable it;
...
/**
	...
	---
*/
template eachMember(char[] mname) {
