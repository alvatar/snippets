module xf.input.writer.X;

private {
	import xf.input.Input;
	import xf.input.KeySym;

	import xf.platform.x11.X;
	import xf.platform.x11.Xlib;
	import xf.platform.x11.Xutil;
	import xf.platform.x11.XKBlib;
}
...
class XInputWriter {
	protected {
		...
		xf.input.KeySym.KeySym _lastDownKey = xf.input.KeySym.KeySym.VoidSymbol;
	}
	...
	bool filter(XEvent ev) {
      ...
      if(cast(uint)kin.unicode < 32) {
        kin.unicode = dchar.init;
			...
			break;
		case KeyRelease:
			_lastDownKey = xf.input.KeySym.KeySym.VoidSymbol;
			KeyboardInput kin;
