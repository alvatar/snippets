...
struct NSEvent {
	...
	int subtype() {
		return cast(int) send(obj, selSubtype);
	}
	...
	bool repeated() {
		return cast(bool) send(obj, selIsARepeat);
	}

	id characters() {
		return send(obj, selCharacters);
	}
	...
	int windowNumber() {
		return cast(int) send(obj, selWindowNumber);
	}
}
